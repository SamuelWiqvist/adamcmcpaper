...
function export_data(problem::Problem, results::Result, jobname::String)
  ...
  writetable("./DWPSDE model/Results/output_param"*jobname*".csv", convert(DataFrame, data_param))
  ...
  writetable("./DWPSDE model/Results/output_res"*jobname*".csv", convert(DataFrame, data_output))

  writetable("./DWPSDE model/Results/output_prior_dist"*jobname*".csv", convert(DataFrame, prior_parameters))

  writetable("./DWPSDE model/Results/output_prior_dist_type"*jobname*".csv", convert(DataFrame, [1 dist_type]))

  writetable("./DWPSDE model/Results/data_used"*jobname*".csv", convert(DataFrame,data_used))
...
function export_data(problem::gpProblem, results::Result, jobname::String)
  ...
  writetable("./DWPSDE model/Results/output_param_dagp"*jobname*".csv", convert(DataFrame, data_param))
  ...
  data_used = zeros(length(problem.data.Z),2)
  data_used[:,1] = problem.data.Z
  writetable("./DWPSDE model/Results/output_res_dagp"*jobname*".csv", convert(DataFrame, data_output))

  writetable("./DWPSDE model/Results/output_prior_dist_dagp"*jobname*".csv", convert(DataFrame, prior_parameters))

  writetable("./DWPSDE model/Results/output_prior_dist_type_dagp"*jobname*".csv", convert(DataFrame, [1 dist_type]))

  writetable("./DWPSDE model/Results/data_used_dagp"*jobname*".csv", convert(DataFrame,data_used))
